...
/**
 * \file    referenceletters/admin/setup.php
 * \ingroup referenceletters
 * \brief   Referenceletters setup page.
 */
...
// Translations
$langs->loadLangs(array("admin", "referenceletters@referenceletters"));

// Initialize technical object to manage hooks of page. Note that conf->hooks_modules contains array of hook context
$hookmanager->initHooks(array('referenceletterssetup', 'globalsetup'));
...
// Setup conf REFERENCELETTERS_MYPARAM1 as a simple string input
$item = $formSetup->newItem('REFERENCELETTERS_MYPARAM1');
$item->defaultFieldValue = 'default value';

// Setup conf REFERENCELETTERS_MYPARAM1 as a simple textarea input but we replace the text of field title
$item = $formSetup->newItem('REFERENCELETTERS_MYPARAM2');
$item->nameText = $item->getNameText().' more html text ';

// Setup conf REFERENCELETTERS_MYPARAM3
$item = $formSetup->newItem('REFERENCELETTERS_MYPARAM3');
$item->setAsThirdpartyType();

// Setup conf REFERENCELETTERS_MYPARAM4 : exemple of quick define write style
$formSetup->newItem('REFERENCELETTERS_MYPARAM4')->setAsYesNo();

// Setup conf REFERENCELETTERS_MYPARAM5
$formSetup->newItem('REFERENCELETTERS_MYPARAM5')->setAsEmailTemplate('thirdparty');

// Setup conf REFERENCELETTERS_MYPARAM6
$formSetup->newItem('REFERENCELETTERS_MYPARAM6')->setAsSecureKey()->enabled = 0; // disabled

// Setup conf REFERENCELETTERS_MYPARAM7
//$formSetup->newItem('REFERENCELETTERS_MYPARAM7')->setAsProduct();
...
$item->helpText = $langs->transnoentities('REFERENCELETTERS_MYPARAM8');


// Setup conf REFERENCELETTERS_MYPARAM9
$formSetup->newItem('REFERENCELETTERS_MYPARAM9')->setAsSelect($TField);


// Setup conf REFERENCELETTERS_MYPARAM10
$item = $formSetup->newItem('REFERENCELETTERS_MYPARAM10');
$item->setAsColor();
...
if ($action == 'updateMask') {
	...
	foreach ($dirmodels as $reldir) {
		$file = dol_buildpath($reldir."core/modules/referenceletters/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file)) {
			$filefound = 1;
	...
	if ($filefound) {
		...
		if ($module->write_file($tmpobject, $langs) > 0) {
			header("Location: ".DOL_URL_ROOT."/document.php?modulepart=referenceletters-".strtolower($tmpobjectkey)."&file=SPECIMEN.pdf");
			return;
		} else {
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'REFERENCELETTERS_'.strtoupper($tmpobjectkey)."_ADDON";
...
print dol_get_fiche_head($head, 'settings', $langs->trans($page_name), -1, "referenceletters@referenceletters");

// Setup page goes here
echo '<span class="opacitymedium">'.$langs->trans("ReferencelettersSetupPage").'</span><br><br>';
					...
					while (($file = readdir($handle)) !== false) {
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php') {
							...
							if ($module->isEnabled()) {
								...
								$constforvar = 'REFERENCELETTERS_'.strtoupper($myTmpObjectKey).'_ADDON';
