...
class pdf_rfltr_contract extends ModelePDFReferenceLetters
{
	...
	function write_file($object, $instance_letter, $outputlangs) {
		...
		if ($conf->referenceletters->dir_output) {
			...
			if (file_exists($dir)) {
				...
				$hookmanager->executeHooks('afterPDFCreation', $parameters, $this, $action); // Note that $action and $object may have been modified by some hooks
	...
	function _pagehead($object, $showaddress, $instance_letter) {
		...
		$this->pdf->SetXY($posx, $posy);
		$this->pdf->SetTextColor(0, 0, 60);
		$this->pdf->MultiCell(100, 3, $this->outputlangs->transnoentities("Date") . " : " . dol_print_date($object->date_contrat, "day", false, $this->outputlangs, true), '', 'R');
