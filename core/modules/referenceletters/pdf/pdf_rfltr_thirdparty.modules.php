...
class pdf_rfltr_thirdparty extends ModelePDFReferenceLetters
{
	...
	function _pagehead($object, $showaddress, $instance_letter) {
		...
		if ($object->code_client) {
			...
			$this->pdf->SetXY($posx, $posy);
			$this->pdf->SetTextColor(0, 0, 60);
			$this->pdf->MultiCell(100, 3, $this->outputlangs->transnoentities("CustomerCode") . " : " . $this->outputlangs->transnoentities($object->code_client), '', 'R');
		}
		...
		if ($showaddress) {
			...
			$carac_client_name = $this->outputlangs->convToOutputCharset($object->name);
			
			$carac_client = pdf_build_address($this->outputlangs, $this->emetteur, $object, 0, 0, 'target');
