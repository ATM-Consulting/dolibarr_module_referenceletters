...
/**
 *   	\file       referenceletters_list.php
 *		\ingroup    referenceletters
 *		\brief      List page for referenceletters
 */
...
// load referenceletters libraries
require_once __DIR__.'/class/referenceletters.class.php';
...
$contextpage = GETPOST('contextpage', 'aZ') ? GETPOST('contextpage', 'aZ') : 'referenceletterslist'; // To manage different context of search
...
$hookmanager->initHooks(array('referenceletterslist')); // Note that conf->hooks_modules contains array
...
if ($enablepermissioncheck) {
	$permissiontoread = $user->rights->referenceletters->read;
	$permissiontoadd = $user->rights->referenceletters->write;
	$permissiontodelete = $user->rights->referenceletters->delete;
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/referenceletters/referenceletters_card.php', 1).'?id='.$id);
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/referenceletters/referenceletters_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);
