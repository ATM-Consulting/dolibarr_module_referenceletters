...
/**
 *   	\file       referenceletterschapters_card.php
 *		\ingroup    referenceletters
 *		\brief      Page to create/edit/view referenceletterschapters
 */
...
$contextpage = GETPOST('contextpage', 'aZ') ? GETPOST('contextpage', 'aZ') : 'referenceletterschapterscard'; // To manage different context of search
...
if ($enablepermissioncheck) {
	$permissiontoread = $user->rights->referenceletters->referenceletterschapters->read;
	$permissiontoadd = $user->rights->referenceletters->referenceletterschapters->write; // Used by the include of actions_addupdatedelete.inc.php and actions_lineupdown.inc.php
	$permissiontodelete = $user->rights->referenceletters->referenceletterschapters->delete || ($permissiontoadd && isset($object->status) && $object->status == $object::STATUS_DRAFT);
	$permissionnote = $user->rights->referenceletters->referenceletterschapters->write; // Used by the include of actions_setnotes.inc.php
	$permissiondellink = $user->rights->referenceletters->referenceletterschapters->write; // Used by the include of actions_dellink.inc.php
} else {
	$permissiontoread = 1;
...
$upload_dir = $conf->referenceletters->multidir_output[isset($object->entity) ? $object->entity : 1].'/referenceletterschapters';
...
if (empty($reshook)) {
	...
	$backurlforlist = dol_buildpath('/referenceletters/referenceletterschapters_list.php', 1);
	...
	$triggersendname = 'REFERENCELETTERS_REFERENCELETTERSCHAPTERS_SENTBYMAIL';
	$autocopy = 'MAIN_MAIL_AUTOCOPY_REFERENCELETTERSCHAPTERS_TO';
	$trackid = 'referenceletterschapters'.$object->id;
...
if (($id || $ref) && $action == 'edit') {
	print load_fiche_titre($langs->trans("ReferenceLettersChapters"), '', 'object_'.$object->picto);
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	...
	$head = referenceletterschaptersPrepareHead($object);
	print dol_get_fiche_head($head, 'card', $langs->trans("ReferenceLettersChapters"), -1, $object->picto);

	$formconfirm = '';

	// Confirmation to delete
	if ($action == 'delete') {
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"].'?id='.$object->id, $langs->trans('DeleteReferenceLettersChapters'), $langs->trans('ConfirmDeleteObject'), 'confirm_delete', '', 0, 1);
	}
	...
	if ($action == 'xxx') {
		$text = $langs->trans('ConfirmActionReferenceLettersChapters', $object->ref);
		/*if (! empty($conf->notification->enabled))
		{
			require_once DOL_DOCUMENT_ROOT . '/core/class/notify.class.php';
			$notify = new Notify($db);
			$text .= '<br>';
			$text .= $notify->confirmMessage('REFERENCELETTERSCHAPTERS_CLOSE', $object->socid, $object);
		}*/
	...
	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="'.dol_buildpath('/referenceletters/referenceletterschapters_list.php', 1).'?restore_lastsearch_values=1'.(!empty($socid) ? '&socid='.$socid : '').'">'.$langs->trans("BackToList").'</a>';
...
//	dol_banner_tab($object, 'ref', $linkback, 1, 'ref', 'ref', $morehtmlref);
	...
	if ($action != 'presend') {
		...
		if ($includedocgeneration) {
			...
			print $formfile->showdocuments('referenceletters:ReferenceLettersChapters', $object->element.'/'.$objref, $filedir, $urlsource, $genallowed, $delallowed, $object->model_pdf, 1, 0, 0, 28, 0, '', '', '', $langs->defaultlang);
		}

		// Show links to link elements
		$linktoelem = $form->showLinkToObjectBlock($object, null, array('referenceletterschapters'));
		...
		$morehtmlcenter = dolGetButtonTitle($langs->trans('SeeAll'), '', 'fa fa-bars imgforviewmode', dol_buildpath('/referenceletters/referenceletterschapters_agenda.php', 1).'?id='.$object->id);
	...
	// Presend form
	$modelmail = 'referenceletterschapters';
